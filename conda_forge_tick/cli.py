...
@click.option(
    "--version-only/--no-version-only",
    default=False,
    help="Only run the version migrator.",
)
@pass_context
def auto_tick(ctx: CliContext, package: Optional[str], version_only: bool) -> None:
    """
    Run the auto-tick process, i.e. execute all migrators including the Version migrator.
    The Version migrator depends on the upstream version information generated by update-upstream-versions.

    If PACKAGE is given, only update that package, otherwise update all packages.
    """
    from . import auto_tick

    auto_tick.main(ctx, package=package, version_only=version_only)
